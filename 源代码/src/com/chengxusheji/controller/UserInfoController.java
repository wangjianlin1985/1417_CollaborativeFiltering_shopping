...
public class UserInfoController extends BaseController {
	...
	public void add(UserInfo userInfo, BindingResult br,
		...
		userInfo.setUserMoney(0.0f); //新用户账号余额为0
	...
	public void OutToExcel(String user_name,String name,String birthDate,String telephone, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        ...
        String[] headers = { "用户名","姓名","性别","出生日期","用户照片","联系电话","邮箱","账户余额","注册时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<userInfoList.size();i++) {
        	UserInfo userInfo = userInfoList.get(i); 
        	dataset.add(new String[]{userInfo.getUser_name(),userInfo.getName(),userInfo.getGender(),userInfo.getBirthDate(),userInfo.getUserPhoto(),userInfo.getTelephone(),userInfo.getEmail(),userInfo.getUserMoney() + "",userInfo.getRegTime()});
        }
        /*
